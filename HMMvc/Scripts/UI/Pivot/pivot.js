var monthsArr = ["ינואר", "פברואר", "מרץ", "אפריל", "מאי", "יוני", "יולי", "אוגוסט", "ספטמבר", "אוקטובר", "נובמבר", "דצמבר"];
...
(function () {
  ...
  callWithJQuery(function($) {
    ...
    aggregators = (function(tpl) {
      return {
        "ספירה": tpl.count(usFmtInt),
        //"Count Unique Values": tpl.countUnique(usFmtInt),
        //"List Unique Values": tpl.listUnique(", "),
        "סכום": tpl.sum(usFmt),
        //"Integer Sum": tpl.sum(usFmtInt),
        "מומצע": tpl.average(usFmt),
        "מינימום": tpl.min(usFmt),
        "מקסימום": tpl.max(usFmt),
        //"Sum over Sum": tpl.sumOverSum(usFmt),
        //"80% Upper Bound": tpl.sumOverSumBound80(true, usFmt),
        //"80% Lower Bound": tpl.sumOverSumBound80(false, usFmt),
        "% סכום מסהכ": tpl.fractionOf(tpl.sum(), "total", usFmtPct),
        "% סכום משורה": tpl.fractionOf(tpl.sum(), "row", usFmtPct),
        "% סכום מעמודה": tpl.fractionOf(tpl.sum(), "col", usFmtPct),
        "% ספירה מסהכ": tpl.fractionOf(tpl.count(), "total", usFmtPct),
        "% ספירה משורה": tpl.fractionOf(tpl.count(), "row", usFmtPct),
        "% ספירה מעמודה": tpl.fractionOf(tpl.count(), "col", usFmtPct)
      };
    })(aggregatorTemplates);
    renderers = {
      "טבלה": function(data, opts) {
        return pivotTableRenderer(data, opts);
      },
      "טבלת גרף": function(data, opts) {
        return $(pivotTableRenderer(data, opts)).barchart();
      },
      "מפת חום": function(data, opts) {
        return $(pivotTableRenderer(data, opts)).heatmap("heatmap", opts);
      },
      "מפת חום לפי שורה": function(data, opts) {
        return $(pivotTableRenderer(data, opts)).heatmap("rowheatmap", opts);
      },
      "מפת חום לפי עמודה": function(data, opts) {
          ...
          return formatString.replace(/%(.)/g, function(m, p) {
            switch (p) {
              ...
              case "d":
                ...
                case "w":
                    return moment(date).week();//dayNames[date["get" + utc + "Day"]()];
              case "x":
    ...
    pivotTableRenderer = function(pivotData, opts) {
      ...
      result.className = "pvtTable shadow";
        ...
        refreshDelayed = (function(_this) {
          return function() {
            ...
            _this.find(".pvtCols li span.pvtAttr").each(function() {
              ...
              //.pvtVals select.pvtAttrDropdown
            _this.find(".pvtVals [data-id=pvtAttrDropdown]").each(function () {
